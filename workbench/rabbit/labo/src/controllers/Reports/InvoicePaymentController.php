...
class InvoicePaymentController extends BaseController
{
    ...
    public function create()
    {
        ...
        return \View::make('labo::report.invoice_payment', compact('date'));
    }
    ...
    public function store()
    {
        ...
        // Start PHPExcel
        $rptName = 'Invoice Payment Report';
        $rptExtension = '.xlsx';
        ...
        if ($input->rptType == 'Default') {
            ...
            $objWorkSheet->getCell('G5')->setValue('Blocked: ' . $filterData->blocked);
            $objWorkSheet->getCell('H6')->setValue($filterData->exchange->usd);
            $objWorkSheet->getCell('I6')->setValue($filterData->exchange->khr);
            ...
            foreach ($rptData as $key => $val) {
                ...
                $objWorkSheet->getCell('B' . $rowNum)->setValue($val->payment_date);
                $objWorkSheet->getCell('C' . $rowNum)->setValue($val->pay_staff_kh_name);
                $objWorkSheet->getCell('D' . $rowNum)->setValue($val->blocked);
                $objWorkSheet->getCell('E' . $rowNum)->setValue($val->invoice_id);
                $objWorkSheet->getCell('F' . $rowNum)->setValue($val->invoice_date);
                $objWorkSheet->getCell('G' . $rowNum)->setValue($val->a_kh_name);
                $objWorkSheet->getCell('H' . $rowNum)->setValue($val->c_kh_name);
                $objWorkSheet->getCell('I' . $rowNum)->setValue($val->overdue_amount);
                $objWorkSheet->getCell('J' . $rowNum)->setValue($val->paid_amount);
            }
            ...
            foreach ($rptData as $key => $val) {
                ...
                $objWorkSheet->getCell('A' . $rowNum)->setValue(($key + 1));
                $objWorkSheet->getCell('B' . $rowNum)->setValue($val->payment_date);
                $objWorkSheet->getCell('C' . $rowNum)->setValue($val->sum_overdue);
                $objWorkSheet->getCell('D' . $rowNum)->setValue($val->sum_paid);
            }
        } // End check report type

        // Export report
        $fileName = $rptName . ' [' . str_replace(':', 'To', $input->dateRange) . ']' . $rptExtension;
        \CpanelExcelWriter::make($objPHPExcel, $fileName);
    }

    private function _reportData($input)
    {
        $data = \DB::table('v_payment')
            ->whereBetween('payment_date', $input->dateRangeExplode);
        ...
        // Check type
        if ($input->rptType == 'Default') {
            $data = $data->orderBy('payment_date')->orderBy('id')
                ->get();
        } else { // For group by date
            $data = $data->select(
                \DB::raw(
                    'payment_date, sum(overdue_amount) as sum_overdue, sum(paid_amount) as sum_paid'
                )
            )
