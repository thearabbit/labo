...
class FeePaymentController extends BaseController
{
    ...
    public function create()
    {
        ...
        return \View::make('labo::report.fee_payment', compact('dateRange'));
    }
    ...
    public function store()
    {
        ...
        if ($input->rptType == 'Default') {
            ...
            foreach ($rptData as $key => $val) {
                ...
                $objWorkSheet->getCell('B' . $rowNum)->setValue($val->fee_date);
                $objWorkSheet->getCell('C' . $rowNum)->setValue($val->fee_staff_kh_name);
            ...
            foreach ($rptData as $key => $val) {
                $rowNum = $startRow + $key;
                $totalFee = '=C' . $rowNum . '+D' . $rowNum;
                $objWorkSheet->getCell('A' . $rowNum)->setValue(($key + 1));
                $objWorkSheet->getCell('B' . $rowNum)->setValue($val->fee_date);
                $objWorkSheet->getCell('C' . $rowNum)->setValue($val->sum_fee_amount);
                $objWorkSheet->getCell('D' . $rowNum)->setValue($val->sum_fee_per);
                $objWorkSheet->getCell('E' . $rowNum)->setValue($totalFee);
            }
    ...
    {
        $data = \DB::table('v_fee')
            ->whereBetween('fee_date', $input->dateRangeExplode);
        ...
        // Check type
        if ($input->rptType == 'Default') {
            $data = $data->orderBy('fee_date')
                ->orderBy('id')
                ->get();
        } else { // For group by date
            $data = $data->select(
                \DB::raw(
                    'fee_date, sum(fee_amount) as sum_fee_amount, sum(fee_per) as sum_fee_per'
                )
            )
                ->groupBy('fee_date')
                ->orderBy('fee_date')
